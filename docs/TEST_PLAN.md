...
- **Observed result:** Not recorded. Automated source-contract tests cannot establish OEM callback behavior, physical routing, or audible output.
...
- **Observed result:** Not recorded. Compose previews and automated tests establish shared layout/state rendering and callback structure only; they do not establish device-specific system-bar appearance, animation rendering, physical routing, audible output, or visual parity on the target phone.
