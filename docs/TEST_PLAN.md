...
- **Steps:** Power ON and confirm `WAITING`, `MODE_NORMAL`, no experiment-owned communication device, prepared state `STATE_INITIALIZED`, play state stopped, one bounded zero-PCM prefill, no matching active local voice-communication/speech playback, and no writer. Start a genuine assistant/speech response and record the generation-scoped monotonic trigger, prepared/prefill, writer-request/first-write, `play()` invocation/return, `PLAYSTATE_PLAYING`, mode-request/observation, device-request/return, and first-earpiece-observation timings. Complete the cycle and confirm ordered cleanup followed by one fresh stopped preparation. Repeat with prefill failure and preparation unavailable/invalid where practical, Power OFF, Close/service shutdown, telephony takeover, Assistant early route qualification, two consecutive sessions, and an assistant gap spanning the existing 7-second linger.
