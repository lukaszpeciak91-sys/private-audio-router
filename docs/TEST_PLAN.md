...
- **Experiment:** Repeat with Assistant early route ON without toggling Private Audio during the conversation. Verify `VOICE_RECOGNITION` remains stable and unsilenced; the prepared silent track reaches `PLAYSTATE_PLAYING` before assistant/speech; no explicit mode/device request or proximity acquisition occurs before speech; and normal protected POC-5 completes afterward.
