...
- **Status:** NOT TESTED
- **Device / Android / build:** Not recorded
- **Steps:** (1) With Main visible, deny or cancel the overlay permission request and verify Main remains foreground. (2) Grant permission and verify Main moves to the background only after the floating window appears, leaving the previous application visible beneath it and Main present in Recents. (3) Drag from the status/background region slowly below and then beyond normal touch slop; verify tap-sized motion does not move the window, a drag updates its position, and every edge remains reachable rather than permitting the controller to move fully off-screen. (4) Tap Power, Expand, and Close without dragging; verify each remains independent and responsive. (5) From another foreground application press Expand repeatedly and verify the existing Main task is revealed without a duplicate and without a routing-state change. (6) show the controller again, press its X, and verify only the overlay is removed while the controller/routing state and Main task remain available. (7) Verify Main Close remains the complete overlay removal, controller shutdown, cleanup, and task-removal path. (8) Repeat drag and edge checks in portrait and landscape where supported, and repeat the established multi-session and telephony checks.
- **Expected result:** The unchanged 300 × 62 dp controller is freely movable from non-control space, uses normal gesture thresholding, and remains within current screen bounds. A confirmed overlay backgrounds rather than finishes Main; denial/failure does not. Expand reuses Main, floating Close remains overlay-only, Main Close remains full shutdown, and no interaction adds routing behavior or another device request.
