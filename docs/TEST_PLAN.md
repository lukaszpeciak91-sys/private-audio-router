...
## Prepared silent-track startup optimization physical gate
...
- **Steps:** Power ON and confirm `WAITING`, `MODE_NORMAL`, no experiment-owned communication device, prepared state `STATE_INITIALIZED`, play state stopped, no matching active local voice-communication/speech playback, and no writer. Start a genuine assistant/speech response and record trigger, `play()`, `PLAYSTATE_PLAYING`, mode-request, and earpiece-request timestamps. Complete the cycle and confirm ordered cleanup followed by one fresh stopped preparation. Repeat with preparation unavailable/invalid where practical, Power OFF, Close/service shutdown, telephony takeover, Fake Phone sonification, two consecutive sessions, and an assistant gap spanning the existing 7-second linger.
- **Expected result:** Genuine routing reuses the prepared track without constructing a second track and preserves `PLAYING` → `MODE_IN_COMMUNICATION` → one earpiece request. Failure falls back to on-demand creation without consuming an attempt or creating `ERROR`. Unused preparation releases without route/mode cleanup. Fake Phone behavior, 1.5-second end confirmation, and 7-second assistant linger remain unchanged.
