...
## Layer 6 final floating-controller physical/emulator gate

- **Status:** NOT TESTED
- **Device / Android / build:** Not recorded
- **Preconditions:** Install the Layer 6 build and retain the approved floating-controller reference. Grant overlay permission through Main when prompted.
- **Steps:** (1) Compare the 300 × 62 dp near-black controller, fine gray outline, restrained corners, spacing, and POWER → STATUS → separator → EXPAND → CLOSE order against the reference; confirm no Hide control or labels beneath Expand/Close. (2) Observe Ready, Waiting, Active, and safely induced Error where practical; verify the approved dot, label, and Power colors without geometry changes. (3) Use Power in each state and verify it matches Main enable/disable behavior and established cleanup. (4) Expand from another foreground app and verify the existing Main task is revealed without controller/routing state change or duplicate task. (5) Close the overlay while enabled and active; verify only the window disappears and routing/service/task continue. (6) Show repeatedly and verify exactly one window. (7) Use Main Close with the overlay visible and verify overlay removal plus complete established controller shutdown and task removal. (8) Repeat permission denial/revocation, process death, multi-session, and real-call safety checks from the Layer 5 and Layer 1.6 gates.
- **Expected result:** The final controller matches the approved reference and consumes only the service projection. Its controls preserve the specified lifecycle boundaries, routing remains service-owned with one production device request, and telephony retains priority.
- **Observed result:** Not recorded. Automated source-contract/build checks cannot establish target-device rendering, overlay touch behavior, Android task behavior, physical routing, or audible output.

## Layer 6.1 floating interaction and task-lifecycle gate

- **Status:** NOT TESTED
- **Device / Android / build:** Not recorded
- **Steps:** (1) With Main visible, deny or cancel the overlay permission request and verify Main remains foreground. (2) Grant permission and verify Main moves to the background only after the floating window appears, leaving the previous application visible beneath it and Main present in Recents. (3) Verify the unchanged 300 × 62 dp controller presents STATUS → POWER → EXPAND → CLOSE, with status left, Power centered, Expand and Close right, and no separator. (4) Drag from the status/background region slowly below and then beyond normal touch slop; verify tap-sized motion does not move the window, a drag updates its position, and every edge remains reachable rather than permitting the controller to move fully off-screen. (5) Tap Power, Expand, and Close without dragging; verify each remains independent and responsive. (6) From another foreground application press Expand and verify the existing Main task is revealed without a duplicate or routing-state change and the overlay is immediately gone. (7) Show the controller again, press its X, and verify only the overlay is removed while the controller/routing state and Main task remain available. (8) Verify Main Close remains the complete overlay removal, controller shutdown, cleanup, and task-removal path. (9) Repeat drag and edge checks in portrait and landscape where supported, and repeat the established multi-session and telephony checks.
- **Expected result:** The unchanged 300 × 62 dp controller is freely movable from non-control space, uses normal gesture thresholding, and remains within current screen bounds. A confirmed overlay backgrounds rather than finishes Main; denial/failure does not. Expand reuses Main and removes the overlay without changing routing state, floating Close remains overlay-only, Main Close remains full shutdown, and no interaction adds routing behavior or another device request.
- **Observed result:** Not recorded. JVM tests validate the clamp calculation and source-level interaction/lifecycle contracts but cannot establish OEM overlay coordinates, system task transitions, touch delivery, physical routing, or audible output.

## Per-app language and existing-Mini configuration gate

- **Status:** NOT TESTED
- **Device / Android / build:** Not recorded
- **Steps:** On API 33 or later, open Settings → Language and select English, Polski, and Default in turn. Confirm the Settings root reports the actual override (Default only when no override exists), Main and Settings use the selected resources, the selection survives process restart through Android's system-managed preference, and Default follows the device language. With one Mini already visible, repeat each transition in Ready, Waiting, Active, and safely induced Error; verify `Ready`/`Waiting`/`Active`/`Error` change to `Gotowe`/`Czeka`/`Aktywne`/`Błąd` and back, including the accessibility description. Confirm the same window remains at the same position, is not duplicated or closed, retains its service binding and controller state, and causes no Power, routing, Expand, or Close action. Confirm a subsequently created or updated foreground notification uses the selected resources. On API 31–32, open Language and verify the Android 13 requirement is shown, Default remains selected, no choice crashes, and the application follows the system language without storing an application override.
- **Expected result:** API 33+ selection is owned by `LocaleManager` and generated locale configuration; clearing the override represents Default. A running Mini refreshes resource-backed presentation in place on the configuration event without changing its window, position, controller/audio state, or controls. API 31–32 remain safely system-language-only pending an explicit compatibility decision.
- **Observed result:** Not recorded. JVM source-contract checks do not establish framework recreation, OEM service configuration delivery, system persistence, notification refresh timing, accessibility output, or physical routing isolation.
