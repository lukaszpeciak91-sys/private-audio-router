...
## Experimental assistant-class routing probe physical gate
...
- **OFF baseline:** Leave “Gemini routing (experimental)” OFF, turn Private Audio ON, start Gemini Live, and make it speak continuously. Confirm the assistant count is visible in diagnostics but no assistant experimental trigger fires, no POC-5 request occurs, and Gemini retains its baseline audible output. Separately regress ChatGPT Voice, proximity, one-request behavior, and Power OFF cleanup.
- **Ordinary-media exclusion:** Turn the experimental preference ON and play YouTube/music with ordinary `USAGE_MEDIA`. Confirm the assistant trigger does not fire and no route request occurs.
- **ON probe:** With the preference ON and Private Audio ON, start Gemini Live from an initial `MODE_NORMAL` state and make it speak continuously. Verify Waiting → Active only if the full protected evidence is established; confirm one POC-5 request, then separately verify speaker → built-in earpiece audibly. Test near/far proximity only after Active evidence exists. Press Private Audio Power OFF and verify cleanup, Ready, and normal audible Gemini speaker routing returns. Copy the diagnostic report before and after cleanup and retain the report plus correlated `dumpsys audio` evidence.
