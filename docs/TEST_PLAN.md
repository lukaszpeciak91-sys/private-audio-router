...
- **Permission UX addition:** From clean app data verify the first missing-access Mini tap shows the Puzru explanation rather than Settings; verify Open settings, Not now, Back, and backdrop behavior; recreate while visible; retry after resolution; and return from Settings with grant and denial/cancellation. Confirm exactly one Mini only after grant, repeat in compact landscape and RTL, and confirm routing/controller state never changes. These runtime/device checks are **NOT TESTED**.
