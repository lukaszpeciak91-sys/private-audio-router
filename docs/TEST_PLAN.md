...
- With Private Audio ON and Assistant early route OFF, run two or three fresh Gemini starts. Compare public recording appearance, disappearance, source, input-device, and silenced transitions against the protected routing generation, silent-track start, mode request, device request, and first earpiece observation.
...
- **Status:** NOT TESTED
- **Preconditions:** Install the diagnostic-format-3 build on a physical device. Record controller state, routing generation, Android mode and communication device, routing-attempt count, prepared/playing silent-track state, cleanup state, and proximity ownership immediately before each capture.
- **Steps:** Generate/save one report (1) in `WAITING`, (2) during an established `ACTIVE` built-in-earpiece session, (3) after meaningful unqualified/no-trigger playback while Puzru remains `WAITING`, and (4) after a completed protected cycle. Compare the before/after state and the report's fresh current observation, latest public-playback section, active cycle ID, and newest-first completed-cycle history. Do not initiate share/email; use only the existing local document flow.
- **Expected result:** Report generation itself changes none of controller state, routing generation, Android mode, communication device, routing-attempt count, prepared/playing track state, cleanup state, or proximity ownership, and creates no routing event. The report nevertheless contains a fresh capture-time observation. The no-trigger case records public Android metadata and qualifier counts without client attribution. The completed case preserves at most the newest three distinct cycle IDs.
- **Evidence rule:** Record Android-reported facts and human-confirmed audible behavior separately. Reported earpiece state does not prove audible output. Do not mark this gate PASS from JVM tests, emulator evidence, or source inspection.
...
- End the external communication session and verify the protected core automatically clears its run before Layer 1.5 arms one fresh waiting experiment. Verify any prepared Waiting track remains stopped with no silence writer, mode request, or device request before a new qualifying session.
- Stop the service cleanly while it is genuinely unbound and verify observer callback unregistration and the existing cleanup. Retain the format-3 final cleanup observation, then separately kill the process and verify no state or completed-cycle history is restored, no automatic re-arm occurs, and `START_NOT_STICKY` does not restart the service.
...
- Verify notification behavior on API 31–32, API 33, and API 34–36, including `specialUse` foreground type on API 34+. From clean app data verify no startup panel, the first missing-grant Power ON explanation, both actions, Back/backdrop dismissal, system grant/denial/dismissal, no repeated explanation or automatic re-request, and activity recreation while visible. Repeat compact-landscape and RTL checks and verify notification visibility and unchanged routing with permission allowed or denied. These runtime/device checks are **NOT TESTED**.

## Layer 1.5 multi-session physical gate

- **Status:** NOT TESTED
- **Device / Android / build / voice-app version:** Not recorded
- **Steps:** (1) Launch Private Audio. (2) Enable once. (3) Start Voice #1 and confirm output through the built-in earpiece. (4) End Voice #1 and confirm route, mode, writer, and silent-track cleanup followed by a fresh armed Waiting state. (5) Do not press Power/Enable again. (6) Start Voice #2 and confirm a fresh POC-5 run moves output to the earpiece. (7) Complete Voice #2 and a third session, then inspect the format-3 report: verify newest-first history contains three distinct stable cycle IDs, each cycle contains exactly one routing request and its trigger/timing/result/final cleanup evidence, and no request occurred between sessions. Complete a fourth session and verify only the oldest cycle is evicted. (8) Disable and confirm protected cleanup, foreground exit, and Ready behavior. (9) Start Voice #5 and verify Private Audio creates no track, mode request, or routing request.
- **Expected result:** One activation spans Voice #1 and Voice #2 as two distinct experiments. Each experiment performs its existing cleanup and contains at most one `setCommunicationDevice()` request. During between-session Waiting, a prepared stopped track may exist under the existing prepared-track architecture, but it is not playing, no silence writer is active, and Waiting alone causes no communication-mode or communication-device request. Disable prevents any action during Voice #3. A telephony/system-priority block or genuine protected failure remains terminal and requires explicit user reset; it must not re-arm.
...
- **Steps:** (1) Open Settings from Main in portrait and verify Main remains unchanged underneath with subtle dimming. (2) Compare the compact 88–92% width graphite sheet, soft corners, thin border, title, row typography, spacing, chevrons, and version placement to the approved portrait reference. (3) Tap outside and verify dismissal. (4) Reopen and press system Back at root; verify dismissal. (5) Rotate to landscape and verify the sheet stays within status, navigation, gesture, and cutout-safe bounds; scroll Root to reach Language, Diagnostics, Advanced, Privacy Policy, About, and Version. (6) Visit Advanced and verify both switches remain reachable and usable; visit Privacy Policy and About and verify all body content and Back remain reachable. (7) Rotate landscape → portrait and verify Settings/Main state remains sane. (8) Tap Save Diagnostic Report from Diagnostics and verify the existing report flow. (9) Repeat while Private Audio is Waiting/Active and verify opening, rotation, scrolling, and Settings navigation cause no new route request or product-state transition. (10) Repeat with an RTL presentation language where practical.
- **Expected result:** Portrait Settings matches the approved modal reference and navigation contract while compact-height Settings removes only the vertical offset needed to remain bounded and scrolls content without reducing typography or touch targets. Clipboard/save content is the existing formatter output, version is build-derived, and Settings introduces no routing/state-projection behavior.
...
- **Permission UX addition:** With access missing, verify every explicit Mini tap shows the Puzru explanation rather than Settings, while startup and resume never show it automatically; verify Open settings, Not now, Back, and backdrop behavior; recreate while visible; and return from Settings with grant and denial/cancellation. Confirm exactly one Mini only after grant, repeat after later revocation and in compact landscape and RTL, and confirm routing/controller state never changes. These runtime/device checks are **NOT TESTED**.
- **Steps:** (1) Tap Main → Mini with access absent and verify Puzru's contextual explanation appears, including the instruction that the next Android screen may be an application list where Puzru must be selected. Verify the panel never appears merely from startup or resume. (2) Exercise Not now, Back, and backdrop dismissal separately; after each, verify no window appears and the next explicit Mini request explains again. (3) Choose Open settings, verify the panel dismisses, then deny or cancel, return to Main, and verify no window or false success appears; explicitly request Mini again and verify the explanation repeats. (4) Choose Open settings again, select Puzru if Android presents the top-level list, grant the relevant access, and return; verify exactly one temporary test surface appears. (5) Tap Mini with permission already granted and verify immediate display without an explanation. (6) Revoke permission where the OS permits and verify the next explicit Mini request explains again without a crash. (7) Remove Mini with its own Close and verify Main, task, and any enabled Puzru controller remain alive and unchanged. (8) Put another application in the foreground and verify Mini remains above it while its owner is alive. (9) Reopen/recreate Main and tap Mini repeatedly; verify no duplicate window. (10) With Mini visible and routing enabled, press Main Close; verify overlay removal, established route/mode/track cleanup, notification removal, and task removal. (11) Kill the process with Mini visible and relaunch; verify it is not automatically restored. (12) Confirm ordinary media and the existing Layer 1.6 multi-session/telephony safety matrix behave unchanged; record Android-reported and audible routing separately.
...
## Assistant early silent-track controlled physical gate

- **Status:** PASS for Xiaomi `2201117TY` / Android 13/API 33; broader compatibility pending
- **Device / Android / build:** Xiaomi `2201117TY` / Android 13/API 33 / Private Audio 0.1.0 (1)
- **Control:** On Xiaomi Android 13, enable Private Audio, leave Assistant early route OFF, reach `WAITING`, begin a fresh Gemini session, ask a short question, record first-response clipping, and save the report.
- **Experiment:** Repeat with Assistant early route ON without toggling Private Audio during the conversation. Verify `VOICE_RECOGNITION` remains stable and unsilenced; the prepared silent track reaches `PLAYSTATE_PLAYING`, then `MODE_IN_COMMUNICATION` is confirmed before assistant/speech; no communication-device request or proximity acquisition occurs before speech; and promotion reuses the track and mode before exactly one protected earpiece request.
- **Timing:** Compare the reported positive PLAYING-to-assistant/speech head-start and preserve assistant/speech-to-earpiece timing. Verify later turns reuse the unchanged 7-second protected context.
- **Safety:** Exercise recording disappearance, Power OFF, preference OFF, observer shutdown, telephony priority, track failure, 10-second timeout, and incompatible communication/browser starts. Each unused pre-arm must release only its track, return to clean `WAITING`, and leave normal communication routing usable.
- **Stop rule:** If recognition becomes unstable, capture the report and stop; do not add retries or extend the timeout. Public metadata, routing evidence, and human-observed audio remain separate evidence.
- **Observed result (supplied 2026-08-22):** The physical/runtime sequence was assistant/sonification → prepared silent track `PLAYING` → early `MODE_IN_COMMUNICATION` → assistant/speech several seconds later → exactly one communication-device request → `ACTIVE`. `VOICE_RECOGNITION` remained present and unsilenced, no device request occurred before speech, and human listening reported that the response beginning was no longer clipped. Additional technical runs reproduced the ordering. This PASS is limited to the recorded Xiaomi Android 13 configuration; other OEMs, Android releases, accessories, telephony boundaries, and the remaining safety matrix are pending.

## Bounded assistant protected-session linger physical gate

- **Status:** PARTIAL — same-cycle reuse physically observed with the earlier 5-second duration; tuned 7-second gate pending.
- **Device / control:** Xiaomi Android 13; Assistant early route OFF.
- **Gemini acceptance:** Complete at least five consecutive conversational turns. Across ordinary inter-turn gaps verify `ACTIVE`, proximity ownership, `MODE_IN_COMMUNICATION`, and the built-in earpiece remain established with the same routing-cycle ID, silent track, and single routing-attempt count. When exact assistant/speech returns within linger, verify diagnostics record resumption/cancellation/reuse without another mode or device request. Abandon a conversation and verify cleanup occurs exactly once after the 1.5-second confirmation plus bounded 7-second linger, then returns to `WAITING` and releases proximity.
...
- **Boundary regression:** Verify browser communication retains its existing end behavior. During an assistant linger separately exercise Private Audio Power OFF and Main Close and verify immediate cleanup with no later timer effect. Where practical, exercise telephony/system priority and required-earpiece/route loss and verify immediate fail-closed cleanup. For speaker, Bluetooth, wired/USB, or another Android-reported replacement, use the generic first-replacement route and loss timestamp; confirm expected cleanup clearing is not marked as active-session route loss.
...
## Prepared silent-track prefill and startup-timing physical gate

- **Status:** NOT TESTED / UNKNOWN
- **Device / control:** Repeat on Xiaomi Android 13 with Assistant early route OFF as the clean reference, then ON without attributing public metadata to an application.
- **Steps:** Power ON and confirm `WAITING`, `MODE_NORMAL`, no experiment-owned communication device, prepared state `STATE_INITIALIZED`, play state stopped, one bounded zero-PCM prefill, no matching active local voice-communication/speech playback, and no writer. Start a genuine assistant/speech response and use the format-3 cycle ID to correlate the generation-scoped monotonic trigger, prepared/prefill, writer-request/first-write, `play()` invocation/return, `PLAYSTATE_PLAYING`, mode-request/observation, device-request/return, recording trace, and first-earpiece-observation timings. Complete the cycle and confirm ordered cleanup followed by one fresh stopped preparation. Repeat with prefill failure and preparation unavailable/invalid where practical, Power OFF, Close/service shutdown, telephony takeover, Assistant early route qualification, two consecutive sessions, and an assistant gap spanning the existing 7-second linger.
- **Expected result:** Prefill alone remains absent from public active-playback diagnostics. Genuine routing reuses the prepared track without constructing a second track and preserves `PLAYING` → `MODE_IN_COMMUNICATION` → one earpiece request. Failure falls back to prepared/on-demand startup without consuming an attempt or creating `ERROR`. Durations are emitted only for the matching genuine generation; Early-pre-arm promotion and stale attempts report unknown/not applicable instead of a cross-attempt duration. Unused preparation releases without route/mode cleanup. Assistant early-route behavior, 1.5-second end confirmation, and 7-second assistant linger remain unchanged.
- **Evidence rule:** Automated tests establish code contracts only. The corrected early-mode sequence and listening result are physically established only for the Xiaomi Android 13 scope recorded above; the remaining preparation/failure matrix and other OEM behavior remain **UNKNOWN** until physical execution.
