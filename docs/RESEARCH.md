...
- **FACT:** The product identity is the modern IANA/BCP-47 language subtag `ckb` (Central Kurdish). It is not one of Java/Android's deprecated ISO aliases (`in`/`id`, `iw`/`he`, or `ji`/`yi`), so Puzru's existing alias map must not be extended. IANA's authoritative registry is <https://www.iana.org/assignments/language-subtag-registry/language-subtag-registry>.
- **FACT:** Android's BCP-47 resource syntax encodes this identity as `values-b+ckb`. Android documents the legacy language/optional-region form and the explicit `b+` BCP-47 form at <https://developer.android.com/guide/topics/resources/providing-resources#AlternativeResources>. Puzru supports API 31 through API 36, entirely newer than the API 21 introduction of BCP-47 resource qualifiers. Therefore `values-b+ckb` is the exact compatible directory for implementation. `values-ckb` is not the selected contract: even if the current resource compiler accepts a three-letter legacy-form qualifier, it is less explicit and provides no compatibility advantage over the BCP-47 form.
- **FACT:** `values-b+ckb+Arab` is valid BCP-47 resource syntax, but it is not necessary for a single general Sorani localization. CLDR assigns Central Kurdish the Arabic script and right-to-left order; see <https://www.unicode.org/cldr/charts/latest/by_type/core_data.html>. Adding `Arab` would intentionally make the product locale script-scoped, cause Puzru's picker to show that scope, and narrow the resource identity without a supported alternate `ckb` script. The logical locale remains `ckb`, with RTL presentation.
- **FACT (repository/JVM verification):** The current Java runtime canonicalizes `Locale.forLanguageTag("ckb")` back to `ckb` without an alias and supplies the native display name `کوردیی ناوەندی`. Puzru canonicalizes generated entries through that API and obtains native picker names from the resulting locale. Platform display data can vary slightly by Android/ICU release, so tests should protect identity and a nonblank self-name rather than freeze host-JDK punctuation or spelling.
- **FACT (implemented follow-through):** Build-time discovery now accepts language-only BCP-47 qualifiers such as `b+ckb`, converts it to logical `ckb`, includes it in app-owned `localeFilters`, and exposes the same generated inventory to runtime code. The Android 13+ picker reads generated `LocaleConfig`, canonicalizes and de-duplicates entries, and has no manual language registry. Android 12/12L continue to follow the system locale, as for every existing product locale.
- **FACT (repository inspection):** Generic contracts discover every app-owned locale for inventory, key/placeholder parity, Permission UX, finalized About copy, and Privacy Summary coverage. Existing instrumentation proves generated `LocaleConfig` equals app-owned inventory and exercises three-letter `b+` families (`bho`, `mai`, and `ceb`). This sufficiently covers the architecture class for `b+ckb`; Central Kurdish has no new legacy-alias or matching class warranting a pre-implementation compatibility test.
- **FACT (repository inspection):** Main, Settings, Permission UX, About and Privacy use direction-aware Compose layout. Mini derives direction from the effective logical application locale, mirrors only direction-sensitive content, uses physical coordinates for its window, and renders status with bidi-aware text layout. Notifications refresh on configuration changes. No special layout, hardcoded language list, bidi markers, or locale-specific branch is needed for `ckb`.
- **INFERENCE:** Standard Android bidi handling should correctly isolate most Sorani prose. The later translation/layout pass must nevertheless inspect mixed runs containing unchanged Latin `Puzru`, email addresses, `EN`, version text, placeholders, punctuation, bullets and numerals. Mini/status and overlay accessibility text carry the highest geometry/bidi risk; Permission UX and long About/Privacy copy carry the highest semantic and paragraph-layout risk. These are validation risks, not architecture defects or justification for manual bidi marks.
- **INFERENCE:** The Translation and independent Localization Audit Skills are sufficient unchanged. Their gates cover native script/orthography, RTL, immutable `Puzru`, localizable `Mini`, ON versus ACTIVE, earpiece versus loudspeaker, supported-voice-app qualifiers, permission/access effects, privacy/data-handling claims, About scope, diagnostics, placeholders, paragraph structure and independent terminology evidence. Central Kurdish terminology for the physical call receiver, routing, “supported” apps, display-over-other-apps access, local processing versus capture/transmission, Mini's four states, and About/Privacy limitations deserves targeted independent audit rather than new architecture.
- **UNKNOWN:** Actual glyph shaping, clipping, mixed-direction presentation, native-name presentation, and resource selection on each supported Android/ICU/OEM build remain runtime matters. When the locale is added, CI/emulator instrumentation should confirm `ckb`, `ckb-IQ`, and `ckb-Arab-IQ` select its sentinel and report RTL; a visual emulator/device pass should cover Main, Mini, Permission UX, About and Privacy. This implementation validation does not block creating the locale.

**Readiness decision:** logical locale `ckb`; resource directory `values-b+ckb`; direction RTL; resource-resolution confidence **PASS**; LocaleConfig integration automatic; picker integration automatic; RTL architecture ready; localization-contract architecture ready. No test architecture or pre-implementation test change is required. During implementation, generic contracts enroll the directory automatically; add only normal locale-specific identity/semantic assertions justified by reviewed terminology, add `ckb` to the existing instrumentation resolution/direction matrix, run Mini measurement, and update supported-locale state/count documentation only after the complete resource tree exists.

**Implementation follow-through:** the locale now uses the recommended qualifier and existing architecture. Resource/source contracts and implementation-time resolution coverage were added; independent linguistic audit and runtime visual validation remain pending.
...
- **FACT (supplied physical result, 2026-08-22):** With the first Assistant early route iteration, `VOICE_RECOGNITION` remained stable and unsilenced and the silent track was `PLAYING` approximately 5–6 seconds before assistant/speech, but the listener still heard approximately the first two spoken words clipped.
- **FACT (supplied physical/runtime evidence, 2026-08-22):** Private Audio 0.1.0 (1) physically executed the corrected early-mode pre-arm on Xiaomi `2201117TY`, Android 13/API 33. The prepared silent track reached `PLAYSTATE_PLAYING` and `MODE_IN_COMMUNICATION` was established several seconds before assistant/speech while the public `VOICE_RECOGNITION` session remained present and `clientSilenced=false`.
- **FACT (supplied physical/runtime evidence, 2026-08-22):** No communication-device request occurred during the early phase. Exact assistant/speech promoted the pre-arm and produced one `setCommunicationDevice(earpiece)` request before `ACTIVE`. Human listening reported that the beginning of the Gemini response was no longer clipped in the successful corrected run; additional technical runs reproduced the early track/mode → speech → single-device-request ordering.
- **INFERENCE:** Early `MODE_IN_COMMUNICATION` is the strongest current explanation for eliminating the previously observed Gemini first-word clipping on this Xiaomi configuration, but the controlled evidence does not establish causal certainty.
...
- **UNKNOWN:** Whether the corrected result generalizes to other devices, OEMs, or Android releases.
