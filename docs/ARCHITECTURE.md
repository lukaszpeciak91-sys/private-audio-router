...
## Experimental sonification-triggered micro-route

The service-owned, default-OFF **Fake Phone pre-arm** preference gates a bounded, provider-independent micro-route; it no longer creates persistent waiting ownership. Clean `WAITING` remains `MODE_NORMAL` with no Fake Phone track or device request. An exact external `USAGE_ASSISTANT` + `CONTENT_TYPE_SONIFICATION` observation starts one silent communication track, confirms playback, requests communication mode, and makes one earpiece request while public state remains `WAITING`.

Exact-sonification disappearance schedules cleanup after 100 ms and reappearance cancels that cleanup. A generation-safe 2,000 ms deadline is fixed at initial detection and cannot be extended by callbacks. Genuine `COMMUNICATION`, `ASSISTANT`, or `BROWSER_COMMUNICATION` evidence promotes a healthy track, mode participation, and request into the normal protected lifecycle without recreating or re-requesting them. An unhealthy handoff cleans only partial optional ownership and continues through ordinary POC-5. Preference/Power/shutdown, telephony priority, earpiece, track, mode, or route loss clean immediately. No focus, volume, retry, capture, application-identity, or package-owner mechanism is introduced.
