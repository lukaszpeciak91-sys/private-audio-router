...
## Experimental assistant-class trigger

The optional assistant probe is an input branch into the existing protected POC-5 execution body, not a broader production trigger or a second routing algorithm. The production branch still requires `MODE_IN_COMMUNICATION`, public `USAGE_VOICE_COMMUNICATION` + `CONTENT_TYPE_SPEECH` playback, and the built-in speaker as the reported communication device. Independently, the default-OFF branch requires an armed controller, its service-owned preference, public `USAGE_ASSISTANT` + `CONTENT_TYPE_SPEECH` playback, no telephony/system-priority mode, and an untouched current-cycle attempt guard. It intentionally permits `MODE_NORMAL` before participation.

Both branches capture pre-change evidence and then share the same ordered silent-track, playing-state confirmation, explicit communication-mode request, mode confirmation, single earpiece request, observation, and cleanup mechanics. Assistant session-end inference follows disappearance of the assistant/speech contribution rather than changing the established voice-communication contribution semantics. Disabling the experimental preference clears only a current assistant-triggered cycle and returns an enabled controller to Waiting. Proximity remains downstream of the same ACTIVE evidence and contains no assistant-specific behavior.
