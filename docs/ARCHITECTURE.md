...
Exact assistant/speech promotes only a healthy playing track, confirmed early mode, and unchanged unsilenced recognition session. Promotion does not replay the track or repeat the mode request; it enters the protected cycle at the existing single earpiece request. Pre-promotion cleanup releases the track and relinquishes only the experiment's mode participation without clearing a communication device.

Exact assistant/speech may promote a healthy already-playing track into the protected `ASSISTANT` POC-5 body without calling `play()` again; explicit mode verification and exactly one earpiece request still follow playback. Communication and browser origins cancel unused pre-arm ownership and use their unchanged standard paths. Recording loss, preference/Power/shutdown, telephony priority, track failure, incompatible playback, or the generation-safe 10-second deadline stops and releases only the track, then restores clean stopped preparation when still enabled. Cleanup does not clear a communication device or force normal mode because the early phase requested neither.
