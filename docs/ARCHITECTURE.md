...
Assistant/speech resumption cancels continuity and reuses the same cycle. Recording disappearance, any change from the captured recognition configuration/session, `clientSilenced=true`, preference OFF, lifecycle shutdown, telephony/system priority, route or mode loss, silent-track failure, or timeout causes immediate existing cleanup. Cycle generation plus a per-continuity token prevents a stale callback from affecting a resumed or newer cycle. Communication and browser-communication lifecycle behavior is not routed through this window.
