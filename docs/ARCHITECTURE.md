...
## Experimental waiting pre-arm

The service-owned, default-OFF **Fake Phone pre-arm** preference is a separate optional layer between clean controller `WAITING` and a real protected routing cycle. While eligible, it prepares one silent communication track, mode participation, and one earpiece request without changing the four-state projection: local participation alone remains `WAITING`. Real provider-independent `COMMUNICATION`, `ASSISTANT`, or `BROWSER_COMMUNICATION` playback evidence promotes the healthy prepared resources into the existing active-session lifecycle without recreating the track or repeating the route request. Optional setup failure cleans partial ownership and leaves normal trigger routing available. No global stream volume, mute, focus, capture, retry, or package-owner mechanism is introduced.
