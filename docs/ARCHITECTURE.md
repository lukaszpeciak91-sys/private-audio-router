...
All three origins converge on one protected POC-5 execution body. Clean waiting prepares a public `AudioTrack` outside the trigger hot path; using `USAGE_VOICE_COMMUNICATION`, `CONTENT_TYPE_SPEECH`, mono PCM 16-bit output, and a device-supported native voice-call sample rate. The body validates and reuses it, or fails open to the prior on-demand creation path, then feeds only zero-valued samples using bounded non-blocking writes on one background-priority thread. After the track reports `PLAYSTATE_PLAYING`, it explicitly requests `MODE_IN_COMMUNICATION` and makes exactly one earpiece request. Observation follows without retry or reassertion, and every exit uses the same reversible cleanup.
...
Layer 1.6 supersedes the Layer 1.5 repeated-one-shot completion callback with a permanent, service-owned controller. Power ON registers one public `AudioPlaybackCallback` and enters clean waiting with one initialized, stopped POC-5 silent track. The prepared track has no writer and owns no active playback, mode participation, device override, `ACTIVE` state, or proximity; preparation failure is non-fatal. A routing cycle starts only when one of the three signatures above qualifies, an earpiece is available, and no telephony/system-priority mode blocks it.

During a cycle, the protected POC-5 sequence is shared and unchanged. Communication and assistant lifecycle inference distinguish external evidence from the known local contribution; browser lifecycle inference follows disappearance of its voice-communication/unknown contribution. A qualifying end must first remain stable for 1.5 seconds, tolerating transient playback recreation. Communication and browser-communication then clean immediately as before. Only an established `ASSISTANT` cycle enters a second, bounded 7-second conversation-linger stage: the existing silent track, communication-mode participation, and earpiece route remain owned, so the authoritative projection remains `ACTIVE` and existing proximity eligibility continues without special handling. Exact assistant/speech resumption cancels the deadline and reuses the same cycle and resources without another routing attempt. A later confirmed disappearance may start a fresh bounded linger. Expiry normally performs the existing ordered cleanup once and returns to clean waiting.

When the separate default-ON **Assistant session continuity** experiment is enabled, healthy expiry of that unchanged linger may instead retain the same `ASSISTANT` cycle for one further non-extending 20-second window. An explicitly stored OFF preference remains OFF. Entry snapshots the current public `VOICE_RECOGNITION` configuration and requires that it be present and unsilenced, along with an initialized playing silent track, `MODE_IN_COMMUNICATION`, the current built-in earpiece, and no telephony/system-priority mode. The window performs no `setMode()`, `setCommunicationDevice()`, track creation, cycle creation, retry, polling, or route reassertion. The protected cycle remains `ACTIVE`, so proximity continues through its existing state-and-route projection.

Assistant/speech resumption cancels continuity and reuses the same cycle. Recording disappearance, any change from the captured recognition configuration/session, `clientSilenced=true`, or preference OFF performs reversible cleanup and returns an otherwise enabled controller to clean `WAITING`; these are normal loss of experimental eligibility, not routing failures. Telephony/system priority and protected route, mode, earpiece, or silent-track failure preserve the established fail-closed `BLOCKED` semantics. Lifecycle shutdown and timeout retain their existing Power-OFF and clean-expiry behavior. Cycle generation plus a per-continuity token prevents a stale callback from affecting a resumed or newer cycle. Communication and browser-communication lifecycle behavior is not routed through this window.

End-confirmation and linger callbacks carry the protected cycle generation, and linger additionally carries per-gap identity; cancellation invalidates both so old work cannot clean a resumed contribution, later gap, later cycle, or post-OFF/ON cycle. Irrelevant playback callbacks do not extend the deadline. Power OFF, Close, controller or observer shutdown, destruction, telephony/system-priority mode, required-earpiece or route loss, silent-track failure, and mode/device ownership loss bypass linger and use immediate protected cleanup. The configurations provide neither package nor client identity. Cleanup invalidates delayed work, clears the route, relinquishes mode, stops the writer, and stops/flushes/releases the track.
...
## Assistant early silent-track experiment

The service-owned, default-ON **Assistant early route** preference gates one provider-independent pre-arm; an explicitly stored OFF preference remains OFF. Only clean enabled `WAITING` with a currently visible public `VOICE_RECOGNITION` recording and exact external `USAGE_ASSISTANT` + `CONTENT_TYPE_SONIFICATION` playback qualifies. Qualification starts the existing prepared/prefilled silent voice-communication/speech track and writer. Only after `PLAYSTATE_PLAYING` is confirmed, the pre-arm establishes and verifies `MODE_IN_COMMUNICATION`; it makes no communication-device request, routing attempt, `ACTIVE` projection, or proximity acquisition.

Exact assistant/speech promotes only a healthy playing track, confirmed early mode, and unchanged unsilenced recognition session. Promotion does not replay the track or repeat the mode request; it enters the protected cycle at the existing single earpiece request. Communication and browser origins cancel unused pre-arm ownership and use their unchanged standard paths. Recording loss, preference/Power/shutdown, telephony priority, track failure, incompatible playback, or the generation-safe 10-second deadline releases the track, safely relinquishes only the pre-arm's owned mode participation, and restores clean stopped preparation when still enabled. Cleanup does not call `clearCommunicationDevice()` because the early phase made no device request.
