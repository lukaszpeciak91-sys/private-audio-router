...
The service-owned, default-OFF **Assistant early route** preference gates one provider-independent pre-arm. Only clean enabled `WAITING` with a currently visible public `VOICE_RECOGNITION` recording and exact external `USAGE_ASSISTANT` + `CONTENT_TYPE_SONIFICATION` playback qualifies. Qualification starts the existing prepared/prefilled silent voice-communication/speech track and writer, but makes no mode request, communication-device request, routing attempt, `ACTIVE` projection, or proximity acquisition.
